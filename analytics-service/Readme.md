...
## dicas para verificar tabela de despesas no banco

```sql
Entrar no banco:
docker exec -it controle-de-gastos-api-main-postgres-analytics-1 psql -U postgres -d analytics_db
Você verá:
analytics_db=#
Listar tabelas
\dt
Esperamos:
public | daily_aggregates | table | postgres
public | expenses | table | postgres
Ver estrutura da tabela
\d expenses
Ver quantidade de despesas          
'SELECT COUNT(*) FROM expenses;
Ver despesas sem expor detalhes sensíveis
SELECT id, user_id, description, amount, currency, category, type, created_at
FROM expenses;
Sair
\q
Sair
